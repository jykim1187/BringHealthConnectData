...
// suspend->코루틴(비동기처리)에서 사용할 수 있는 함수 , fun은 함수 선언 , :List<HeartRateRecord>는 반환 타입
    suspend fun readHeartRates(): List<HeartRateRecord> {
        ...
        val yesterday = now.minusSeconds(60 * 60 * 24) //현재시간에서 24시간 뺀거. 즉 어제

        val request = ReadRecordsRequest(
            recordType = HeartRateRecord::class, //어떤 종류의 기록을 읽을건지(HeartRateRecord)
            timeRangeFilter = TimeRangeFilter.between(yesterday, now) //어느 시간 범위를 읽을 건지->여기선 어제부터 오늘
        )
    ...
    suspend fun readStepCounts(): List<StepsRecord> {
        val now = Instant.now()
        val yesterday = now.minusSeconds(60 * 60 * 24) // 24시간 범위로 설정

        val request = ReadRecordsRequest(
            recordType = StepsRecord::class, // 걸음수 데이터를 읽음
            timeRangeFilter = TimeRangeFilter.between(yesterday, now) // 시간 범위 지정
        )
    ...
    suspend fun readCaloriesBurned(): List<TotalCaloriesBurnedRecord> {
        ...
        val request = ReadRecordsRequest(
            ...
            timeRangeFilter = TimeRangeFilter.between(yesterday, now) // 어제부터 오늘까지의 데이터
        )
